...
VS Code provides two different scopes for settings: _**User** these settings apply globally to any instance of VS Code you open_ **Workspace** these settings are stored inside your workspace in a `.vscode` folder and only apply when the workspace is opened. Settings defined on this scope overwrite the user scope.
...
Depending on your platform, the user settings file is located here: _**Windows** `%APPDATA%\Code\User\settings.json`_ **Mac** `$HOME/Library/Application Support/Code/User/settings.json` * **Linux** `$HOME/.config/Code/User/settings.json`
